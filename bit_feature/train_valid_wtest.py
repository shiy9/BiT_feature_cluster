...
testing_dir = 'data_root/learning/testing_ctrst_flip/folder1'
...
for val_folder_index in range(train_splits):  # Note: with validation: for val_folder_index in range(5):
    ...
    dataset['test'] = datasets.ImageFolder(root=testing_dir, transform=image_transforms['test'])
    ...
    print("Training-set size:", dataset_sizes['train'],
          ...
          '\nTesting-set size:', dataset_sizes['test'])  # Note: valid cmt
        ...
        for phase in ['train', 'valid']:  # Note: orig with valid: for phase in ['train', 'valid']:
            ...
            if phase == 'valid':
                ...
                for inputs, labels in dataloaders['test']:
                    ...
                    preds_list_test = list(np.array(preds.argmax(dim=1).cpu().detach().numpy()))
                    labels_list_test = list(np.array(labels.cpu().detach().numpy()))
                    pred_test.append(preds_list_test)
                    true_test.append(labels_list_test)

                    running_corrects_test += (np.array(preds_list_test) == np.array(labels_list_test)).sum().item()
            ...
            if phase == 'valid':
                valid_cm = confusion_matrix(true, pred)
                print('Validation cm:')
                print(valid_cm)
                pred_test = sum(pred_test, [])
                true_test = sum(true_test, [])
                epoch_acc_test = running_corrects_test / dataset_sizes['test']
                balance_acc_test = balanced_accuracy_score(true_test, pred_test)
                cm_test = confusion_matrix(true_test, pred_test)
                test_cm_list.append(cm_test)
                print(f'{val_folder_index + 1}/{train_splits} Test Acc: {balance_acc_test:.4f}')
            ...
            if phase == 'train':
                ...
                writer.add_scalar(f'Test{val_folder_index}/Balance_accuracy', balance_acc_test, epoch)
                writer.flush()
            ...
            if phase == 'valid' and balance_acc_test > best_test_acc:
                best_test_epoch = epoch
                best_test_split = val_folder_index
                best_test_acc = balance_acc_test
    ...
    np.savetxt(f'{model_dir}/split_{val_folder_index}_cm.txt', test_cm_list)
    ...
    print(f'Best epoch in test is {best_test_epoch} in split {best_test_split}')
